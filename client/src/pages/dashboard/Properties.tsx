...
const propertyTypes = ['All', 'apartment', 'villa', 'penthouse', 'commercial', 'land'];
    ...
    <div className="space-y-5">
      ...
      >
        <div>
          ...
          <p className="text-[12px] text-[var(--color-stone)] mt-0.5">{pagination.total} total properties in your portfolio</p>
        </div>
        <button
      ...
      >
        ...
        <div className="flex items-center gap-1 flex-wrap">
          {propertyTypes.map((type) => (
            <button
              key={type}
              onClick={() => { setFilterType(type); setPagination(p => ({ ...p, page: 1 })); }}
      ...
      <motion.div
        ...
        className="bg-white rounded-xl border border-[var(--color-mist)] overflow-hidden"
      >
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h3 className="text-[15px] font-semibold text-[var(--color-charcoal)] mb-1.5">No properties found</h3>
            <p className="text-[13px] text-[var(--color-stone)] mb-5">Get started by adding your first property.</p>
            <button
              onClick={() => navigate('/dashboard/properties/new')}
              className="flex items-center gap-2 px-4 py-2 bg-[var(--color-charcoal)] text-white text-[13px] font-medium rounded-lg hover:bg-[var(--color-champagne-dark)] transition-colors"
            >
          ...
          <>
            ...
            <div className="grid grid-cols-[1fr_100px_90px_90px_70px_40px] gap-3 px-5 py-2.5 border-b border-[var(--color-mist)] bg-[var(--color-warm-white)]/70">
              <button
                onClick={() => setSortField(f => f === 'title' ? '-title' : 'title')}
              ...
              <span className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[var(--color-stone)]">Type</span>
              <span className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[var(--color-stone)]">Status</span>
              <button
                onClick={() => setSortField(f => f === '-price' ? 'price' : '-price')}
                className="flex items-center gap-1 text-[10px] font-semibold uppercase tracking-[0.1em] text-[var(--color-stone)] text-left"
              >
                Price <ArrowUpDown className="w-2.5 h-2.5" />
              </button>
              <span className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[var(--color-stone)]">Area</span>
              <span />
            </div>
              ...
              >
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-[13px] font-medium text-[var(--color-charcoal)] truncate leading-tight">{prop.title}</p>
                    <p className="text-[11px] text-[var(--color-stone)] mt-0.5">{prop.city}, {prop.state}</p>
                  </div>
                </div>

                <span className="text-[11px] capitalize text-[var(--color-stone)] font-medium">{prop.propertyType}</span>

                <StatusBadge status={prop.status} />

                <span className="text-[13px] font-medium text-[var(--color-charcoal)]" style={{ fontFamily: 'var(--font-display)' }}>
                  {formatPrice(prop.price)}
                </span>

                <span className="text-[11px] text-[var(--color-stone)]">{prop.area}</span>

                {/* Actions */}
                <div className="relative flex justify-end">
                  <button
                    onClick={() => setActiveMenu(activeMenu === prop._id ? null : prop._id)}
                    ...
                    {activeMenu === prop._id && (
                      ...
                      >
                        <button className="w-full flex items-center gap-2 px-3 py-1.5 text-[13px] text-[var(--color-charcoal)] hover:bg-[var(--color-warm-white)]">
                          <Eye className="w-3.5 h-3.5 text-[var(--color-stone)]" /> View
                        </button>
                        <button className="w-full flex items-center gap-2 px-3 py-1.5 text-[13px] text-[var(--color-charcoal)] hover:bg-[var(--color-warm-white)]">
                          <Pencil className="w-3.5 h-3.5 text-[var(--color-stone)]" /> Edit
                        </button>
                        <button
